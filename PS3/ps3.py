    ...
    if "*" in word :
     for L in VOWELS:
      if word.replace('*',L) in word_list:
         for x in word :
            x=x.lower()
            if x not in hand.keys() and L == 'u' :
               if word.count(x) > hand[x] and L == 'u' :
                return False
            else :
              continue    
         return True       
...
def calculate_handlen(hand):
    ...
    returns: integer
    """
    handlen = len(hand.keys())
    return handlen

def play_hand(hand, word_list,turn):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while True :
     ...
     if hand == {} :
        print('\nRan out of letters. Total score for this hand : ',total_score,' points') 
        break
    return total_score
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while (times):
     ...
     oldhand=hand.copy()
     score = play_hand(hand,word_list,1)
     if n==0 :
      s=input('Would you like to replay the hand? ')
      if s.lower()=='yes':
        n+=1
        score2 = play_hand(oldhand,word_list,0)
        if score >= score2 :
            total_score +=score 
            continue
        else :
